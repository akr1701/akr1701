# 👋 Hi, I'm Ankit Kumar  
### 💻 Software Developer | Full-Stack | Python • JavaScript • SQL  

<p align="center">
  <img src="https://readme-typing-svg.herokuapp.com?color=00F7FF&center=true&vCenter=true&lines=Software+Developer;Full+Stack+Enthusiast;Building+Real+World+Applications;Always+Learning+New+Tech" />
</p>

---

## 🧑‍💻 About Me  
- 💻 Passionate Software Developer with strong problem-solving skills  
- 🚀 Building real-world full-stack applications  
- 🌐 Exploring MERN Stack & backend development  
- 📊 Strong foundation in Data Analytics (Python, SQL, Visualization)  
- 🎯 Goal: To become a skilled Full-Stack Developer  

---

## ⚙️ Tech Stack  

### 💡 Languages  
- Python | JavaScript | SQL | HTML | CSS  

### 🌐 Frontend  
- HTML | CSS | React.js (Learning)  

### 🔧 Backend  
- Node.js (Learning) | Python  

### 🗄️ Database  
- MySQL | MongoDB  

### 🛠️ Tools  
- Git | GitHub | VS Code | Postman  

---

## 🚀 Projects  

### 🔹 Data Analysis Web App (Full-Stack) ⭐  
- Built a full-stack web app to analyze datasets and generate insights  
- Features: Upload dataset, data cleaning, visualization, chart generation  
- Interactive UI for better user experience  
- Backend APIs for processing and analytics  
- Tech: React.js, Node.js, Python, SQL  

---

### 🔹 Student Management System  
- Developed a system to manage student data with admin control  
- Features: Search, Update, Delete operations  
- Tech: Python, MongoDB  

---

### 🔹 Cricket Score Prediction App  
- Built a prediction system using data analysis techniques  
- Focused on real-time insights and accuracy  
- Tech: Python  

---

### 🔹 Amazon Sales Dashboard  
- Developed dashboard to analyze business performance  
- Improved insights using data visualization  
- Tech: Power BI, Excel  

---

## 💼 Experience  
- 💻 Data Analyst Intern – PW Skills  
- 🎓 NATS Trainee – MIT Muzaffarpur  

---

## 🎓 Education  
- 🎓 B.Tech in Computer Science (2023)  
- Darbhanga College of Engineering  

---

## 📜 Certifications  
- Python Programming – Cisco  
- C/C++ – E & ICT Academy IIT Kanpur  
- Soft Skills – NPTEL  

---

## 🌐 Connect With Me  
- 💼 LinkedIn: https://www.linkedin.com/in/ankit-kumar-084941220  
- 📧 Email: akr200101@gmail.com  

---

## ⚡ Fun Fact  
> I build software 💻 and turn ideas into real-world applications 🚀  

---

⭐ *Open to Software Developer opportunities*
